...
func RunCLI(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if len(args) < 1 {
		logger.Error(ctx, logging.EventACMRun, "stage", "parse", "ok", false, "error_code", v1.ErrCodeMissingSubcommand)
	...
	default:
		...
		logger.Error(ctx, logging.EventACMRun, "stage", "parse", "subcommand", args[0], "ok", false, "error_code", v1.ErrCodeUnknownSubcommand)
...
func runWithIO(ctx context.Context, logger logging.Logger, args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if err := fs.Parse(args); err != nil {
		...
		logger.Error(ctx, logging.EventACMRun, "stage", "parse_flags", "subcommand", "run", "ok", false, "error_code", v1.ErrCodeInvalidFlags)
		return 2
	}

	in, closeInput, err := openInputWithStdin(*inPath, stdin)
	if err != nil {
		logger.Error(ctx, logging.EventACMIORead, "stage", "open_input", "subcommand", "run", "ok", false, "path", *inPath, "error_code", v1.ErrCodeReadFailed)
	...
	if err != nil {
		logger.Error(ctx, logging.EventACMRun, "stage", "service_init", "subcommand", "run", "ok", false, "error_code", v1.ErrCodeServiceInitFailed)
...
func validateWithIO(ctx context.Context, logger logging.Logger, args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if err := fs.Parse(args); err != nil {
		...
		logger.Error(ctx, logging.EventACMRun, "stage", "parse_flags", "subcommand", "validate", "ok", false, "error_code", v1.ErrCodeInvalidFlags)
		return 2
	}

	in, closeFn, err := openInputWithStdin(*inPath, stdin)
	if err != nil {
		logger.Error(ctx, logging.EventACMIORead, "stage", "open_input", "subcommand", "validate", "ok", false, "path", *inPath, "error_code", v1.ErrCodeReadFailed)
		fmt.Fprintf(stderr, "failed to open input: %v\n", err)
		return 2
	}
	defer closeFn()
	...
	b, err := io.ReadAll(in)
	if err != nil {
		logger.Error(ctx, logging.EventACMIORead, "stage", "read_input", "subcommand", "validate", "ok", false, "error_code", v1.ErrCodeReadFailed)
