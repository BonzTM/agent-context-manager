...
5. Use `/acm-verify ...` before `/acm-report ...` for any code, config, schema, or executable behavior change.
...
7. Use `/acm-report ...` to close the task; it may enforce additional task keys from `.acm/acm-workflows.yaml`.
8. Use `/acm-memory ...` for durable decisions and gotchas.
9. Use `/acm-eval ...` only for retrieval-quality maintenance, not task completion.
