...
func main() {
	...
	if len(os.Args) < 2 {
		logger.Error(ctx, logging.EventCtxMCP, "stage", "parse", "ok", false, "error_code", "MISSING_SUBCOMMAND")
		usage()
		os.Exit(2)
	}
	...
	switch os.Args[1] {
	case "tools":
		logger.Info(ctx, logging.EventCtxMCP, "stage", "start", "subcommand", "tools")
		printJSON(map[string]any{"version": v1.Version, "tools": mcp.ToolDefinitions()})
		logger.Info(ctx, logging.EventCtxMCP, "stage", "finish", "subcommand", "tools", "exit_code", 0)
		os.Exit(0)
	case "invoke":
	...
	case "--help", "-h", "help":
		...
		os.Exit(0)
	default:
		logger.Error(ctx, logging.EventCtxMCP, "stage", "parse", "subcommand", os.Args[1], "ok", false, "error_code", "UNKNOWN_SUBCOMMAND")
...
func invoke(ctx context.Context, logger logging.Logger, args []string) int {
	...
	logger.Info(ctx, logging.EventCtxMCP, "stage", "start", "subcommand", "invoke")
	...
	if err := fs.Parse(args); err != nil {
		logger.Error(ctx, logging.EventCtxMCP, "stage", "parse_flags", "subcommand", "invoke", "ok", false, "error_code", "INVALID_FLAGS")
		return 2
	}
	if *tool == "" {
		logger.Error(ctx, logging.EventCtxMCP, "stage", "parse_flags", "subcommand", "invoke", "ok", false, "error_code", "MISSING_TOOL")
	...
	if err != nil {
		logger.Error(ctx, logging.EventCtxIORead, "stage", "read_input", "subcommand", "invoke", "ok", false, "path", *in, "error_code", "READ_FAILED")
		printJSON(map[string]any{"ok": false, "error": map[string]any{"code": "READ_FAILED", "message": err.Error()}})
		return 1
	}
	logger.Info(ctx, logging.EventCtxIORead, "stage", "read_input", "subcommand", "invoke", "ok", true, "path", *in, "bytes", len(input))

	svc, closeService, err := runtime.NewServiceFromEnvWithLogger(ctx, logger)
	if err != nil {
		logger.Error(ctx, logging.EventCtxMCP, "stage", "service_init", "subcommand", "invoke", "ok", false, "error_code", "SERVICE_INIT_FAILED")
	...
	if apiErr != nil {
		logger.Error(ctx, logging.EventCtxMCP, "stage", "invoke", "subcommand", "invoke", "ok", false, "tool", *tool, "error_code", apiErr.Code)
		printJSON(map[string]any{"ok": false, "error": apiErr.ToPayload()})
		return 1
	}
	printJSON(map[string]any{"ok": true, "result": result})
	logger.Info(ctx, logging.EventCtxMCP, "stage", "finish", "subcommand", "invoke", "tool", *tool, "exit_code", 0)
	return 0
}
