...
5. On completion, run `/acm-report ...`.
6. If plan tracking is active, post `work` updates using `receipt_id` (with optional `plan_key`) and prefer `tasks` (legacy `items` accepted). For status checks, send zero tasks.
7. If a durable discovery was made, run `/acm-memory ...`.
