---
name: acm-broker
description: Use the agent-context-manager broker (CLI or MCP) to load repo-owned rules and durable state, post work/review updates, and close tasks with deterministic JSON contracts.
---
...
- These commands assume installed `acm` and `acm-mcp` binaries are available on `PATH`. An optional `acm-web` binary provides a read-only web dashboard for humans at `http://localhost:8080/` (kanban board, status).
- Preferred CLI path:
  - `acm context ...`
  - `acm fetch ...`
  - `acm work ...`
