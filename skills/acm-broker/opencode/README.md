...
When a task specifically needs rendered ACM artifacts instead of normal envelopes, use the backend-only `export` surface through `acm run --in assets/requests/export.json`. The `acm-mcp` binary now uses the JSON-RPC stdio protocol. For export via MCP, send a JSON-RPC `tools/call` request to the running server. For quick human-facing CLI output, `context`, `fetch`, `history`, and `status` also support `--format json|markdown` with optional `--out-file` / `--force`; those flags lower to the same backend export path.
